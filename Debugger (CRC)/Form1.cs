...
namespace Debugger__CRC_
{
    public partial class frmCRC : Form
    {
        ...
        // Méthode qui calcule le CRC d'une donnée.
        {
            ...
            CRCVal = 0; // manquais cette initialisation
            // Parcourir la donnée
            for (int i = 0; i < Donnée.Length; i++)
            {
                char c = Donnée[i];
                if ((c >= '0') && (c <= '9')) // C'est un chiffre ( manquais les 2 "=")
                {
                    NbChiffres++;
                    CRCVal = CRCVal + ((int)c - (int)'0');
                    if (CRCVal >= 100) // On ne peut pas dépasser 100 parce qu'on n'a que deux chiffres pour le CRC ( manquais un "=")
                        CRCVal = CRCVal - 100;
                }
            }
